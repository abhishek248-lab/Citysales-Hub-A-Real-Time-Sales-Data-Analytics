...
# Helper function to read existing CSV files and get max order_id
def get_last_order_id(volume_path):
    try:
        df = spark.read.schema(order_schema).option("header", "true").csv(volume_path)
        result = df.agg(spark_max(col("order_id"))).collect()
...
def generate_bhubaneswar_order(start_id):
    """Generate realistic orders for Bhubaneswar store"""
    data = []
    ...
    for _ in range(num_rows):
        ...
        amount = round(random.uniform(100.0, 30000.0), 2)
...
def generate_khordha_order(start_id):
    """Generate realistic orders for Khordha store"""
    data = []
    num_rows = random.randint(8, 15)
...
def generate_product_data():
    ...
    for _ in range(num_products):
        ...
        product_name = random.choice(["laptop", "phone", "tablet", "watch", "camera", "headphones", "tv", "speaker", 
                                       ...
                                       "sofa", "chair", "table", "bed", "desk", "bookshelf", "cabinet", "lamp"])
...
def generate_randomcustomer():
    ...
    for _ in range(num_rows):
        ...
        customer_name = f"{random.choice(FIRST_NAMES)} {random.choice(LAST_NAMES)}"
        region = random.choice(ODISHA_REGIONS)
...
# Read existing data to continue from last checkpoint
bhubaneswar_order_id = get_last_order_id(bhubaneswar_volume) + 1
khordha_order_id = get_last_order_id(khordha_volume) + 1
