...
def price_demand_analysis():
    ...
    # Join orders with CURRENT product prices only (not temporal join)
    # Since orders doesn't track history, we join with current product versions
    current_products = df_products.filter(col("__END_AT").isNull())
    ...
    # Calculate demand changes (comparing current with previous price period)
    w2 = Window.partitionBy("product_id").orderBy("current_price")
    
    df_final = (
        df_demand
        .withColumn("prev_demand", lag("current_demand").over(w2))
        .withColumn(
            "demand_direction",
